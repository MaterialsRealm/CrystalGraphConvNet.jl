...
[CrystalGraphConvNet](https://github.com/MaterialsRealm/CrystalGraphConvNet.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("CrystalGraphConvNet")
   ```

3. Run

   ```julia-repl
   julia> using CrystalGraphConvNet
   ```
...
version of CrystalGraphConvNet, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MaterialsRealm/CrystalGraphConvNet.jl"
```
...
our [GitHub repository](https://github.com/MaterialsRealm/CrystalGraphConvNet.jl)
for new releases.
Once we release a new version, you can update CrystalGraphConvNet by typing

```@repl
using Pkg
Pkg.update("CrystalGraphConvNet")
Pkg.gc()
```
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("CrystalGraphConvNet")

   julia> Pkg.gc()
   ```

2. Press `Ctrl+D` to quit the current session. Start a new Julia session and
   reinstall CrystalGraphConvNet.
