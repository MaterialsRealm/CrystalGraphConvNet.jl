...
!!! info
    The style guide is always a work in progress, and not all CrystalGraphConvNet code
    follows the rules. When modifying CrystalGraphConvNet, please fix the style violations
...
CrystalGraphConvNet uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MaterialsRealm/CrystalGraphConvNet.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the CrystalGraphConvNet directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to CrystalGraphConvNet have
